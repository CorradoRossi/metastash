...
export default function useUpdateAssetOnchainBalance() {
  ...
  const updateAssetOnchainBalance = useCallback(
    async (assetToUpdate, accountAddress, successCallback) => {
      const balance = await getOnchainAssetBalance(assetToUpdate, accountAddress);
      if (balance?.amount !== assetToUpdate?.balance?.amount) {
        ...
        logger.log(`balance updated with onchain data for asset ${assetToUpdate.symbol}`, balance);
      }
    },
    [allAssets, dispatch]
  );
